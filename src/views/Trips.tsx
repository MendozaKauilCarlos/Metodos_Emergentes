...
import { subscribeDriverTrips, type Trip } from '../services/tripsRepository';

const mockTrips: Trip[] = [
  {
    id: '1',
    status: 'COMPLETADO',
    driverId: 'demo-user-123',
    passengerName: 'Pasajero',
    coordinates: '21.1390, -86.8349',
    address:
      'Instituto Tecnológico Superior de Felipe Carrillo Puerto, Calle Diagonal 63, Felipe Carrillo Puerto, Quintana Roo, 77200, México',
    time: '19:26',
    price: '$ $$42',
  },
  {
...
export default function Trips() {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeDriverTrips(
      user.uid,
      (list) => {
        setTrips(list);
        setLoadingTrips(false);
      ...
      (err) => {
        ...
        setFirestoreError(
          err.message.includes('index')
            ? 'Firestore: falta crear el índice compuesto. Ejecuta firebase deploy o el enlace que aparece en la consola del navegador.'
            : err.message
        );
        setLoadingTrips(false);
      }
  ...
  const filteredTrips = useMemo(() => {
    ...
    return byTab.filter(
      (trip) =>
        trip.address.toLowerCase().includes(q) ||
        trip.passengerName.toLowerCase().includes(q) ||
        trip.coordinates.toLowerCase().includes(q)
    );
  }, [trips, activeTab, searchQuery]);

  const totalTrips = trips.length;
  const activeTripsCount = trips.filter(
    (t) => t.status === 'EN_PROGRESO' || t.status === 'SOLICITADO'
  ).length;
  const completedTripsCount = trips.filter((t) => t.status === 'COMPLETADO').length;

  const driverLabel = (trip: Trip): string => {
    if (trip.driverId && trip.driverId !== 'undefined') return trip.driverId.slice(0, 8) + '…';
    return '—';
  };
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5] dark:bg-zinc-900 text-[#2d3748] dark:text-zinc-100 pb-24 font-sans transition-colors duration-200">
      ...
      <main className="p-4 md:p-8 max-w-[1200px] mx-auto mt-4">
        ...
        {!isDriver && (
          <p className="text-[#718096] dark:text-zinc-400 mb-6">
            Esta vista muestra viajes del conductor. Inicia sesión como conductor o usa la demo de conductor.
          </p>
        )}
        ...
        {loadingTrips && isDriver ? (
          <p className="text-center py-10 text-[#718096] dark:text-zinc-400">Cargando viajes desde Firestore…</p>
        ) : (
          <>
            ...
            <div className="space-y-4">
              {filteredTrips.map((trip) => (
                <div
                  key={trip.id}
                  className="bg-white dark:bg-zinc-800 rounded-xl p-5 shadow-sm border border-gray-100 dark:border-zinc-700 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-colors duration-200"
                >
                  <div className="flex items-center gap-4 md:w-1/3">
                    <div
                      className={`px-4 py-1.5 rounded-full flex items-center gap-2 text-white text-[12px] font-bold ${
                    ...
                    <div
                      className={`px-4 py-1.5 rounded-full text-white text-[11px] font-bold tracking-wider ${
                        trip.status === 'CANCELADO' ? 'bg-[#f59e0b]' : 'bg-[#00d4aa]'
                      }`}
                    >
                      {trip.status}
                  ...
                  <div className="flex flex-col gap-1.5 md:w-5/12 text-[12px] text-[#718096] dark:text-zinc-400">
                    ...
                    <div className="font-bold text-[#00d4aa] text-[14px] mt-1">{trip.price}</div>
                  </div>
                </div>
              ))}
